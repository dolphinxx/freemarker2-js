...
export class ExtendedDecimalFormatParser {
    ...
    static PARAM_HANDLERS : HashMap; public static PARAM_HANDLERS_$LI$() : HashMap { ExtendedDecimalFormatParser.__static_initialize(); return ExtendedDecimalFormatParser.PARAM_HANDLERS; };

    static __static_initializer_0() {
        let m : HashMap = <any>(new Map<any, any>());
    ...
    static parse(formatString : string, locale : string) : DecimalFormat {
    ...
    newUnknownParameterException(name : string, namePos : number) : Error {
        ...
        let legalNames : Set = /* keySet */((m) => { let r=[]; m.forEach((v, k, m) => r.push(k)); return r; })(<any>ExtendedDecimalFormatParser.PARAM_HANDLERS_$LI$());
        let legalNameArr : Array<any> = /* toArray */legalNames.slice(0);
    ...
    constructor(formatString : string, locale : string) {
