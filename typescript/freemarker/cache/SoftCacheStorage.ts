...
export class SoftCacheStorage implements ConcurrentCacheStorage, CacheStorageWithGetSize {
    ...
    // /*private*/ queue : List = <any>(new List());
    ...
    public get(key : any) : any {
        // this.processQueue();
        // let ref : /*Reference*/any = /*<Reference>*//* get */this.map.get(key);
    ...
    //     for(; ; ) {
    //
    //         let ref : SoftCacheStorage.SoftValueReference = <SoftCacheStorage.SoftValueReference>(()=> {if(this.queue.size() === 0) return null;const result = this.queue.get(this.queue.size() - 1);this.queue.splice(this.queue.size() - 1, 1);return result;})();
    //         if(ref == null) {
    //             return;
    //         }
    ...
    //         if(this.concurrent) {
    //             try {
    //                 /* invoke */SoftCacheStorage.atomicRemove_$LI$().apply(this.map, [key, ref]);
    //             } catch(__e) {
...
export namespace SoftCacheStorage {
    ...
    //     key : any;
    //
    //     constructor(key : any, value : any, queue : List) {
