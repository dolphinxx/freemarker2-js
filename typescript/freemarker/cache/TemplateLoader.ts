...
export interface TemplateLoader {
     ...
     * 
     * @param {String} name The name (template root directory relative path) of the template, already localized and normalized by
     ...
     * separators.
     * @return {Object} An object representing the template source, which can be supplied in subsequent calls to
     ...
     * {@code null} then!
     */
    findTemplateSource(name : string) : any;
     ...
     * 
     * @param {Object} templateSource an object representing a template source, obtained through a prior call to
     ...
     * @return {number} the time of last modification of the specified template source, or -1 if the time is not known.
     */
    getLastModified(templateSource : any) : number;
     ...
     * {link #findTemplateSource(String)}. This must be an object on which
     * {link TemplateLoader#closeTemplateSource(Object)} wasn't applied yet.
     * @param {String} encoding       the character encoding used to translate source bytes to characters. Some loaders may not have access
     ...
     * @return {Reader} A {link Reader} representing the template character stream; not {@code null}. It's the responsibility of
     ...
     */
    getReader(templateSource : any, encoding : string) : Reader;
     ...
     * 
     * @param {Object} templateSource the template source that should be closed.
     */
    closeTemplateSource(templateSource : any);
}
