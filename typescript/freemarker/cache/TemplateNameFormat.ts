/* Generated from Java with JSweet 2.2.0-SNAPSHOT - http://www.jsweet.org */
import {MalformedTemplateNameException} from '../template/MalformedTemplateNameException';
import {StringUtil} from '../template/utility/StringUtil';

/**
 * Symbolizes a template name format, which defines the basic syntax of names through algorithms such as normalization.
 * The API of this class isn't exposed as it's too immature, so custom template name formats aren't possible yet.
 * 
 * @since 2.3.22
 * @class
 */
export abstract class TemplateNameFormat {
    constructor() {
    }

    /**
     * The default template name format when {link Configuration#Configuration(Version) incompatible_improvements} is
     * below 2.4.0. As of FreeMarker 2.4.0, the default {@code incompatible_improvements} is still {@code 2.3.0}, and it
     * will certainly remain so for a very long time. In new projects it's highly recommended to use
     * {link #DEFAULT_2_4_0} instead.
     */
    public static DEFAULT_2_3_0 : TemplateNameFormat; public static DEFAULT_2_3_0_$LI$() : TemplateNameFormat { if(TemplateNameFormat.DEFAULT_2_3_0 == null) TemplateNameFormat.DEFAULT_2_3_0 = new TemplateNameFormat.Default020300(); return TemplateNameFormat.DEFAULT_2_3_0; };

    /**
     * The default template name format only when {link Configuration#Configuration(Version) incompatible_improvements}
     * is set to 2.4.0 (or higher). This is not the out-of-the-box default format of FreeMarker 2.4.x, because the
     * default {@code incompatible_improvements} is still 2.3.0 there.
     * <p>
     * <p>
     * Differences to the {link #DEFAULT_2_3_0} format:
     * 
     * <ul>
     * 
     * <li>The scheme and the path need not be separated with {@code "://"} anymore, only with {@code ":"}. This makes
     * template names like {@code "classpath:foo.ftl"} interpreted as an absolute name with scheme {@code "classpath"}
     * and absolute path "foo.ftl". The scheme name before the {@code ":"} can't contain {@code "/"}, or else it's
     * treated as a malformed name. The scheme part can be separated either with {@code "://"} or just {@code ":"} from
     * the path. Hence, {@code myscheme:/x} is normalized to {@code myscheme:x}, while {@code myscheme:///x} is
     * normalized to {@code myscheme://x}, but {@code myscehme://x} or {@code myscheme:/x} aren't changed by
     * normalization. It's up the {link TemplateLoader} to which the normalized names are passed to decide which of
     * these scheme separation conventions are valid (maybe both).</li>
     * 
     * <li>{@code ":"} is not allowed in template names, except as the scheme separator (see previous point).
     * 
     * <li>Malformed paths throw {link MalformedTemplateNameException} instead of acting like if the template wasn't
     * found.
     * 
     * <li>{@code "\"} (backslash) is not allowed in template names, and causes {link MalformedTemplateNameException}.
     * With {link #DEFAULT_2_3_0} you would certainly end up with a {link TemplateNotFoundException} (or worse,
     * it would work, but steps like {@code ".."} wouldn't be normalized by FreeMarker).
     * 
     * <li>Template names might end with {@code /}, like {@code "foo/"}, and the presence or lack of the terminating
     * {@code /} is seen as significant. While their actual interpretation is up to the {link TemplateLoader},
     * operations that manipulate template names assume that the last step refers to a "directory" as opposed to a
     * "file" exactly if the terminating {@code /} is present. Except, the empty name is assumed to refer to the root
     * "directory" (despite that it doesn't end with {@code /}).
     * 
     * <li>{@code //} is normalized to {@code /}, except of course if it's in the scheme name terminator. Like
     * {@code foo//bar///baaz.ftl} is normalized to {@code foo/bar/baaz.ftl}. (In general, 0 long step names aren't
     * possible anymore.)</li>
     * 
     * <li>The {@code ".."} bugs of the legacy normalizer are fixed: {@code ".."} steps has removed the preceding
     * {@code "."} or {@code "*"} or scheme steps, not treating them specially as they should be. Now these work as
     * expected. Examples: {@code "a/./../c"} has become to {@code "a/c"}, now it will be {@code "c"}; {@code "a/b/*}
     * {@code /../c"} has become to {@code "a/b/c"}, now it will be {@code "a/*}{@code /c"}; {@code "scheme://.."} has
     * become to {@code "scheme:/"}, now it will be {@code null} ({link TemplateNotFoundException}) for backing out of
     * the root directory.</li>
     * 
     * <li>As now directory paths has to be handled as well, it recognizes terminating, leading, and lonely {@code ".."}
     * and {@code "."} steps. For example, {@code "foo/bar/.."} now becomes to {@code "foo/"}</li>
     * 
     * <li>Multiple consecutive {@code *} steps are normalized to one</li>
     * 
     * </ul>
     */
    public static DEFAULT_2_4_0 : TemplateNameFormat; public static DEFAULT_2_4_0_$LI$() : TemplateNameFormat { if(TemplateNameFormat.DEFAULT_2_4_0 == null) TemplateNameFormat.DEFAULT_2_4_0 = new TemplateNameFormat.Default020400(); return TemplateNameFormat.DEFAULT_2_4_0; };

    /**
     * Converts a name to a template root directory based name, so that it can be used to find a template without
     * knowing what (like which template) has referred to it. The rules depend on the name format, but a typical example
     * is converting "t.ftl" with base "sub/contex.ftl" to "sub/t.ftl".
     * 
     * @param {String} baseName   Maybe a file name, maybe a directory name. The meaning of file name VS directory name depends on the
     * name format, but typically, something like "foo/bar/" is a directory name, and something like
     * "foo/bar" is a file name, and thus in the last case the effective base is "foo/" (i.e., the directory
     * that contains the file). Not {@code null}.
     * @param {String} targetName The name to convert. This usually comes from a template that refers to another template by name. It
     * can be a relative name, or an absolute name. (In typical name formats absolute names start with
     * {@code "/"} or maybe with an URL scheme, and all others are relative). Not {@code null}.
     * @return {String} The path in template root directory relative format, or even an absolute name (where the root directory
     * is not the real root directory of the file system, but the imaginary directory that exists to store the
     * templates). The standard implementations shipped with FreeMarker always return a root relative path
     * (except if the name starts with an URI schema, in which case a full URI is returned).
     */
    abstract toRootBasedName(baseName : string, targetName : string) : string;

    /**
     * Normalizes a template root directory based name (relative to the root or absolute), so that equivalent names
     * become equivalent according {link String#equals(Object)} too. The rules depend on the name format, but typical
     * examples are "sub/../t.ftl" to "t.ftl", "sub/./t.ftl" to "sub/t.ftl" and "/t.ftl" to "t.ftl".
     * 
     * <p>The standard implementations shipped with FreeMarker always returns a root relative path
     * (except if the name starts with an URI schema, in which case a full URI is returned), for example, "/foo.ftl"
     * becomes to "foo.ftl".
     * 
     * @param {String} name The root based name (a name that's either absolute or relative to the root). Not {@code null}.
     * @return {String} The normalized root based name. Not {@code null}.
     */
    abstract normalizeRootBasedName(name : string) : string;

    /**
     * Converts a root based name to an absolute name, which is useful if you need to pass a name to something that
     * doesn't necessary resolve relative paths relative to the root (like the {@code #include} directive).
     * 
     * @param {String} name The root based name (a name that's either absolute or relative to the root). Not {@code null}.
     * @return {String}
     */
    abstract rootBasedNameToAbsoluteName(name : string) : string;

    static checkNameHasNoNullCharacter(name : string) {
        if(name.indexOf(String.fromCharCode(0)) !== -1) {
            throw new MalformedTemplateNameException(name, "Null character (\\u0000) in the name; possible attack attempt");
        }
    }

    static newRootLeavingException(name : string) : MalformedTemplateNameException {
        return new MalformedTemplateNameException(name, "Backing out from the root directory is not allowed");
    }
}
TemplateNameFormat["__class"] = "freemarker.cache.TemplateNameFormat";


export namespace TemplateNameFormat {

    export class Default020300 extends TemplateNameFormat {
        /**
         * 
         * @param {String} baseName
         * @param {String} targetName
         * @return {String}
         */
        toRootBasedName(baseName : string, targetName : string) : string {
            if(targetName.indexOf("://") > 0) {
                return targetName;
            } else if(/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(targetName, "/")) {
                let schemeSepIdx : number = baseName.indexOf("://");
                if(schemeSepIdx > 0) {
                    return baseName.substring(0, schemeSepIdx + 2) + targetName;
                } else {
                    return targetName.substring(1);
                }
            } else {
                if(!/* endsWith */((str, searchString) => { let pos = str.length - searchString.length; let lastIndex = str.indexOf(searchString, pos); return lastIndex !== -1 && lastIndex === pos; })(baseName, "/")) {
                    baseName = baseName.substring(0, baseName.lastIndexOf("/") + 1);
                }
                return baseName + targetName;
            }
        }

        /**
         * 
         * @param {String} name
         * @return {String}
         */
        normalizeRootBasedName(name : string) : string {
            TemplateNameFormat.checkNameHasNoNullCharacter(name);
            let path : string = name;
            for(; ; ) {
                let parentDirPathLoc : number = path.indexOf("/../");
                if(parentDirPathLoc === 0) {
                    throw TemplateNameFormat.newRootLeavingException(name);
                }
                if(parentDirPathLoc === -1) {
                    if(/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(path, "../")) {
                        throw TemplateNameFormat.newRootLeavingException(name);
                    }
                    break;
                }
                let previousSlashLoc : number = path.lastIndexOf('/', parentDirPathLoc - 1);
                path = path.substring(0, previousSlashLoc + 1) + path.substring(parentDirPathLoc + "/../".length);
            }
            for(; ; ) {
                let currentDirPathLoc : number = path.indexOf("/./");
                if(currentDirPathLoc === -1) {
                    if(/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(path, "./")) {
                        path = path.substring("./".length);
                    }
                    break;
                }
                path = path.substring(0, currentDirPathLoc) + path.substring(currentDirPathLoc + "/./".length - 1);
            }
            if(path.length > 1 && (c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(path.charAt(0)) == '/'.charCodeAt(0)) {
                path = path.substring(1);
            }
            return path;
        }

        /**
         * 
         * @param {String} name
         * @return {String}
         */
        rootBasedNameToAbsoluteName(name : string) : string {
            if(name.indexOf("://") > 0) {
                return name;
            }
            if(!/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(name, "/")) {
                return "/" + name;
            }
            return name;
        }

        /**
         * 
         * @return {String}
         */
        public toString() : string {
            return "TemplateNameFormat.DEFAULT_2_3_0";
        }

        constructor() {
            super();
        }
    }
    Default020300["__class"] = "freemarker.cache.TemplateNameFormat.Default020300";


    export class Default020400 extends TemplateNameFormat {
        /**
         * 
         * @param {String} baseName
         * @param {String} targetName
         * @return {String}
         */
        toRootBasedName(baseName : string, targetName : string) : string {
            if(this.findSchemeSectionEnd(targetName) !== 0) {
                return targetName;
            } else if(/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(targetName, "/")) {
                let targetNameAsRelative : string = targetName.substring(1);
                let schemeSectionEnd : number = this.findSchemeSectionEnd(baseName);
                if(schemeSectionEnd === 0) {
                    return targetNameAsRelative;
                } else {
                    return baseName.substring(0, schemeSectionEnd) + targetNameAsRelative;
                }
            } else {
                if(!/* endsWith */((str, searchString) => { let pos = str.length - searchString.length; let lastIndex = str.indexOf(searchString, pos); return lastIndex !== -1 && lastIndex === pos; })(baseName, "/")) {
                    let baseEnd : number = baseName.lastIndexOf("/") + 1;
                    if(baseEnd === 0) {
                        baseEnd = this.findSchemeSectionEnd(baseName);
                    }
                    baseName = baseName.substring(0, baseEnd);
                }
                return baseName + targetName;
            }
        }

        /**
         * 
         * @param {String} name
         * @return {String}
         */
        normalizeRootBasedName(name : string) : string {
            TemplateNameFormat.checkNameHasNoNullCharacter(name);
            if(name.indexOf('\\') !== -1) {
                throw new MalformedTemplateNameException(name, "Backslash (\"\\\") is not allowed in template names. Use slash (\"/\") instead.");
            }
            let scheme : string;
            let path : string;
            {
                let schemeSectionEnd : number = this.findSchemeSectionEnd(name);
                if(schemeSectionEnd === 0) {
                    scheme = null;
                    path = name;
                } else {
                    scheme = name.substring(0, schemeSectionEnd);
                    path = name.substring(schemeSectionEnd);
                }
            }
            if(path.indexOf(':') !== -1) {
                throw new MalformedTemplateNameException(name, "The \':\' character can only be used after the scheme name (if there\'s any), not in the path part");
            }
            path = this.removeRedundantSlashes(path);
            path = this.removeDotSteps(path);
            path = this.resolveDotDotSteps(path, name);
            path = this.removeRedundantStarSteps(path);
            return scheme == null?path:scheme + path;
        }

        findSchemeSectionEnd(name : string) : number {
            let schemeColonIdx : number = name.indexOf(":");
            if(schemeColonIdx === -1 || name.lastIndexOf('/', schemeColonIdx - 1) !== -1) {
                return 0;
            } else {
                if(schemeColonIdx + 2 < name.length && (c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(name.charAt(schemeColonIdx + 1)) == '/'.charCodeAt(0) && (c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(name.charAt(schemeColonIdx + 2)) == '/'.charCodeAt(0)) {
                    return schemeColonIdx + 3;
                } else {
                    return schemeColonIdx + 1;
                }
            }
        }

        removeRedundantSlashes(path : string) : string {
            let prevName : string;
            do {
                prevName = path;
                path = StringUtil.replace$java_lang_String$java_lang_String$java_lang_String(path, "//", "/");
            } while((prevName !== path));
            return /* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(path, "/")?path.substring(1):path;
        }

        removeDotSteps(path : string) : string {
            let nextFromIdx : number = path.length - 1;
            findDotSteps: while((true)) {
                let dotIdx : number = path.lastIndexOf('.', nextFromIdx);
                if(dotIdx < 0) {
                    return path;
                }
                nextFromIdx = dotIdx - 1;
                if(dotIdx !== 0 && (c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(path.charAt(dotIdx - 1)) != '/'.charCodeAt(0)) {
                    continue findDotSteps;
                }
                let slashRight : boolean;
                if(dotIdx + 1 === path.length) {
                    slashRight = false;
                } else if((c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(path.charAt(dotIdx + 1)) == '/'.charCodeAt(0)) {
                    slashRight = true;
                } else {
                    continue findDotSteps;
                }
                if(slashRight) {
                    path = path.substring(0, dotIdx) + path.substring(dotIdx + 2);
                } else {
                    path = path.substring(0, path.length - 1);
                }
            }
        }

        /**
         * @param {String} name The original name, needed for exception error messages.
         * @param {String} path
         * @return {String}
         * @private
         */
        resolveDotDotSteps(path : string, name : string) : string {
            let nextFromIdx : number = 0;
            findDotDotSteps: while((true)) {
                let dotDotIdx : number = path.indexOf("..", nextFromIdx);
                if(dotDotIdx < 0) {
                    return path;
                }
                if(dotDotIdx === 0) {
                    throw TemplateNameFormat.newRootLeavingException(name);
                } else if((c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(path.charAt(dotDotIdx - 1)) != '/'.charCodeAt(0)) {
                    nextFromIdx = dotDotIdx + 3;
                    continue findDotDotSteps;
                }
                let slashRight : boolean;
                if(dotDotIdx + 2 === path.length) {
                    slashRight = false;
                } else if((c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(path.charAt(dotDotIdx + 2)) == '/'.charCodeAt(0)) {
                    slashRight = true;
                } else {
                    nextFromIdx = dotDotIdx + 3;
                    continue findDotDotSteps;
                }
                let previousSlashIdx : number;
                let skippedStarStep : boolean = false;
                {
                    let searchSlashBacwardsFrom : number = dotDotIdx - 2;
                    scanBackwardsForSlash: while((true)) {
                        if(searchSlashBacwardsFrom === -1) {
                            throw TemplateNameFormat.newRootLeavingException(name);
                        }
                        previousSlashIdx = path.lastIndexOf('/', searchSlashBacwardsFrom);
                        if(previousSlashIdx === -1) {
                            if(searchSlashBacwardsFrom === 0 && (c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(path.charAt(0)) == '*'.charCodeAt(0)) {
                                throw TemplateNameFormat.newRootLeavingException(name);
                            }
                            break scanBackwardsForSlash;
                        }
                        if((c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(path.charAt(previousSlashIdx + 1)) == '*'.charCodeAt(0) && (c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(path.charAt(previousSlashIdx + 2)) == '/'.charCodeAt(0)) {
                            skippedStarStep = true;
                            searchSlashBacwardsFrom = previousSlashIdx - 1;
                        } else {
                            break scanBackwardsForSlash;
                        }
                    }
                }
                path = path.substring(0, previousSlashIdx + 1) + (skippedStarStep?"*/":"") + path.substring(dotDotIdx + (slashRight?3:2));
                nextFromIdx = previousSlashIdx + 1;
            }
        }

        removeRedundantStarSteps(path : string) : string {
            let prevName : string;
            removeDoubleStarSteps: do {
                let supiciousIdx : number = path.indexOf("*/*");
                if(supiciousIdx === -1) {
                    break removeDoubleStarSteps;
                }
                prevName = path;
                if((supiciousIdx === 0 || (c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(path.charAt(supiciousIdx - 1)) == '/'.charCodeAt(0)) && (supiciousIdx + 3 === path.length || (c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(path.charAt(supiciousIdx + 3)) == '/'.charCodeAt(0))) {
                    path = path.substring(0, supiciousIdx) + path.substring(supiciousIdx + 2);
                }
            } while((prevName !== path));
            if(/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(path, "*")) {
                if(path.length === 1) {
                    path = "";
                } else if((c => c.charCodeAt==null?<any>c:c.charCodeAt(0))(path.charAt(1)) == '/'.charCodeAt(0)) {
                    path = path.substring(2);
                }
            }
            return path;
        }

        /**
         * 
         * @param {String} name
         * @return {String}
         */
        rootBasedNameToAbsoluteName(name : string) : string {
            if(this.findSchemeSectionEnd(name) !== 0) {
                return name;
            }
            if(!/* startsWith */((str, searchString, position = 0) => str.substr(position, searchString.length) === searchString)(name, "/")) {
                return "/" + name;
            }
            return name;
        }

        /**
         * 
         * @return {String}
         */
        public toString() : string {
            return "TemplateNameFormat.DEFAULT_2_4_0";
        }

        constructor() {
            super();
        }
    }
    Default020400["__class"] = "freemarker.cache.TemplateNameFormat.Default020400";

}




TemplateNameFormat.DEFAULT_2_4_0_$LI$();

TemplateNameFormat.DEFAULT_2_3_0_$LI$();
