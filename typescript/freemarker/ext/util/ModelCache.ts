...
export abstract class ModelCache {
    ...
    /*private*/ modelCache : Map<any, any> = null;

    /*private*/ refQueue : ReferenceQueue = null;
    ...
    public setUseCache(useCache : boolean) {
        ...
        if(useCache) {
            this.modelCache = <any>(new IdentityHashMap<any, ModelCache.ModelReference>());
            this.refQueue = <any>(new ReferenceQueue<TemplateModel>());
        } else {
    ...
    public clearCache() {
        if(this.modelCache != null) {
            {
                /* clear */(<any>this.modelCache).clear();
            };
        }
    }
    ...
    register(model : TemplateModel, object : any) {
        {
            for(; ; ) {
                let queuedRef : ModelCache.ModelReference = <ModelCache.ModelReference>this.refQueue.poll();
                if(queuedRef == null) {
                    break;
                }
                /* remove */this.modelCache.delete(queuedRef.object);
            };
            /* put */this.modelCache.set(object, (() => { let __o : any = new ModelCache.ModelReference(model, object, this.refQueue); __o.__delegate = new ModelCache.ModelReference(model, object, this.refQueue); return __o; })());
        };
    }
}
...
export namespace ModelCache {
    ...
    export class ModelReference {
        ...
        constructor(ref : TemplateModel, object : any, refQueue : ReferenceQueue) {
