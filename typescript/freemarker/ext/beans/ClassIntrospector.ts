...
import { _JavaVersions } from '../../core/_JavaVersions';
...
import { _Java8 } from '../../core/_Java8';
...
export class ClassIntrospector {
    ...
    /*private*/ cache : Map<any, any> = <any>(<Map<any, Map>>new Map<any, Map>(0, 0.75, 16));

    /*private*/ cacheClassNames : Set = <any>([]);

    /*private*/ classIntrospectionsInProgress : Set = <any>([]);

    /*private*/ modelFactories : List = <any>([]);

    /*private*/ modelFactoriesRefQueue : ReferenceQueue = <any>(new ReferenceQueue<any>());
    ...
    public constructor(builder? : any, sharedLock? : any, hasSharedInstanceRestrictons? : any, shared? : any) {
        if(((builder != null && builder instanceof <any>ClassIntrospectorBuilder) || builder === null) && ((sharedLock != null) || sharedLock === null) && ((typeof hasSharedInstanceRestrictons === 'boolean') || hasSharedInstanceRestrictons === null) && ((typeof shared === 'boolean') || shared === null)) {
            ...
            this.cache = <any>(<Map<any, Map>>new Map<any, Map>(0, 0.75, 16));
            this.cacheClassNames = <any>([]);
            this.classIntrospectionsInProgress = <any>([]);
            this.modelFactories = <any>([]);
            this.modelFactoriesRefQueue = <any>(new ReferenceQueue<any>());
            if(this.exposureLevel===undefined) this.exposureLevel = 0;
            if(this.exposeFields===undefined) this.exposeFields = false;
            if(this.methodAppearanceFineTuner===undefined) this.methodAppearanceFineTuner = null;
            if(this.methodSorter===undefined) this.methodSorter = null;
     ...
     * @private
     */
    createClassIntrospectionData(clazz : any) : Map {
        ...
        if(/* size */((m) => { let r=0; m.forEach((v, k, m) => r++); return r; })(<any>introspData) > 1) {
            ...
            let e : Entry = /* iterator */((a) => { var i = 0; return { next: function() { return i<a.length?a[i++]:null; }, hasNext: function() { return i<a.length; }}})(/* entrySet */((m) => { if(m.entries==null) m.entries=[]; return m.entries; })(<any>introspData)).next();
            return /* singletonMap */(k => { let o = {entries: [{getKey: function() { return this.key }, getValue: function() { return this.value },key:k, value:e.getValue()}]}; return o; })(e.getKey());
        }
    }

    addFieldsToClassIntrospectionData(introspData : Map, clazz : any) {
    ...
    addBeanInfoToClassIntrospectionData(introspData : Map, clazz : any, accessibleMethods : Map) {
        let beanInfo : BeanInfo = Introspector.getBeanInfo(clazz);
        let pdas : List = this.getPropertyDescriptors(beanInfo, clazz);
        ...
        if(this.exposureLevel < BeansWrapper.EXPOSE_PROPERTIES_ONLY) {
            ...
            let mds : List = this.getMethodDescriptors(beanInfo, clazz);
                ...
                if(method != null && this.isAllowedToExpose(method)) {
                    ...
                    if(methodKey != null) {
                        ...
                        if(previous != null && (previous instanceof Function)) {
                            ...
                            let replaced : Array<any> = /* put */ClassIntrospector.getArgTypesByMethod(introspData).set(method, method.getParameterTypes());
                            if(replaced != null) {
     ...
     * @private
     */
    getPropertyDescriptors(beanInfo : BeanInfo, clazz : any) : List {
        let introspectorPDsArray : Array<any> = beanInfo.getPropertyDescriptors();
        let introspectorPDs : List = introspectorPDsArray != null?/* asList */introspectorPDsArray.slice(0):/* emptyList */[];
        if(!this.treatDefaultMethodsAsBeanMembers || _JavaVersions.JAVA_8_$LI$() == null) {
            return introspectorPDs;
        }
        let mergedPRMPs : LinkedHashMap = null;
        {
    ...
    mergeInPropertyDescriptor(mergedPRMPs : LinkedHashMap, pd : PropertyDescriptor) {
    ...
    mergeInPropertyReaderMethodPair(mergedPRMPs : LinkedHashMap, propName : string, newPRM : ClassIntrospector.PropertyReaderMethodPair) {
    ...
    mergeInPropertyReaderMethod(mergedPRMPs : LinkedHashMap, propName : string, readerMethod : Function) {
    ...
    putIfMergedPropertyReaderMethodPairDiffers(mergedPRMPs : LinkedHashMap, propName : string, replaced : any, newPRMP : ClassIntrospector.PropertyReaderMethodPair) {
     ...
     * @private
     */
    getMethodDescriptors(beanInfo : BeanInfo, clazz : any) : List {
        let introspectorMDArray : Array<any> = beanInfo.getMethodDescriptors();
        let introspectionMDs : List = introspectorMDArray != null && introspectorMDArray.length !== 0?/* asList */introspectorMDArray.slice(0):/* emptyList */[];
        if(!this.treatDefaultMethodsAsBeanMembers || _JavaVersions.JAVA_8_$LI$() == null) {
            ...
            for(let index154=0; index154 < array155.length; index154++) {
                let method = array155[index154];
                {
                    if(_JavaVersions.JAVA_8_$LI$().isDefaultMethod(method) && !method.isBridge()) {
                        ...
                        let overloads : List = /* get */defaultMethodsToAddByName.get(/* getName */method.name);
                        if(overloads == null) {
                            overloads = <any>([]);
        ...
        let newIntrospectionMDs : ArrayList = <any>([]);
    ...
    containsMethodWithSameParameterTypes(overloads : List, m : Function) : boolean {
        if(overloads == null) {
            return false;
        }
    ...
    addPropertyDescriptorToClassIntrospectionData(introspData : Map, pd : PropertyDescriptor, clazz : any, accessibleMethods : Map) {
    ...
    addGenericGetToClassIntrospectionData(introspData : Map, accessibleMethods : Map) {
    ...
    addConstructorsToClassIntrospectionData(introspData : Map, clazz : any) {
        try {
            let ctors : Array<any> = clazz.getConstructors();
            if(ctors.length === 1) {
                let ctor : Constructor = ctors[0];
    ...
    static discoverAccessibleMethods$java_lang_Class(clazz : any) : Map {
    ...
    public static discoverAccessibleMethods$java_lang_Class$java_util_Map(clazz : any, accessibles : Map) {
        if(Modifier.isPublic(clazz.getModifiers())) {
            try {
                ...
                for(let i : number = 0; i < methods.length; i++) {
                    ...
                    let methodList : List = /* get */accessibles.get(sig);
                    if(methodList == null) {
                        methodList = <any>([]);
    ...
    static getMatchingAccessibleMethod(m : Function, accessibles : Map) : Function {
        if(m == null) {
            return null;
        }
        let sig : ClassIntrospector.MethodSignature = new ClassIntrospector.MethodSignature(m);
        let ams : List = /* get */accessibles.get(sig);
        if(ams == null) {
            return null;
        }
    ...
    static getFirstAccessibleMethod(sig : ClassIntrospector.MethodSignature, accessibles : Map) : Function {
        let ams : List = /* get */accessibles.get(sig);
    ...
    static getArgTypesByMethod(classInfo : Map) : Map {
        let argTypes : Map<any, any> = <Map><any>/* get */classInfo.get(ClassIntrospector.ARG_TYPES_BY_METHOD_KEY_$LI$());
        if(argTypes == null) {
    ...
    forcedClearCache() {
        {
            ...
            /* clear */(this.cacheClassNames.length = 0);
    ...
    static getArgTypes(classInfo : Map, method : Function) : Array {
        let argTypesByMethod : Map<any, any> = <Map><any>/* get */classInfo.get(ClassIntrospector.ARG_TYPES_BY_METHOD_KEY_$LI$());
     ...
     * @return {Set}
     */
    keySet(clazz : any) : Set {
        let set : Set = <any>(/* keySet */((m) => { let r=[]; m.forEach((v, k, m) => r.push(k)); return r; })(<any>this.get(clazz)).slice(0));
        /* remove */(a => { let index = a.indexOf(ClassIntrospector.CONSTRUCTORS_KEY_$LI$()); if(index>=0) { a.splice(index, 1); return true; } else { return false; }})(set);
        /* remove */(a => { let index = a.indexOf(ClassIntrospector.GENERIC_GET_KEY_$LI$()); if(index>=0) { a.splice(index, 1); return true; } else { return false; }})(set);
        /* remove */(a => { let index = a.indexOf(ClassIntrospector.ARG_TYPES_BY_METHOD_KEY_$LI$()); if(index>=0) { a.splice(index, 1); return true; } else { return false; }})(set);
        return set;
    }
     ...
     * @return {Array}
     */
    getRegisteredModelFactoriesSnapshot() : Array {
        {
