...
import {BuiltInsForNumbers} from "../../../freemarker/core/BuiltInsForNumbers";
import number_to_dateBI = BuiltInsForNumbers.number_to_dateBI;
...
export class Matcher {
    ...
    acceptMode: number = Matcher.NOANCHOR;
     ...
     */
    transparentBounds: boolean = false;

    /**
     * If anchoringBounds is true then the boundaries of this
     * matcher's region match anchors such as ^ and $.
     */
    anchoringBounds: boolean = true;

    /**
     * All matchers have the state used by Pattern during a match.
     */
    constructor(parent?: Pattern, text?: string) {
        if (arguments.length === 2) {
            this.parentPattern = parent;
            this.text = text;

            // Allocate state storage
            let parentGroupCount = Math.max(parent.capturingGroupCount||0, 10);
            this.groups = new Array<number>(parentGroupCount * 2);
            this.locals = new Array<number>(parent.localCount);
...
// }
     ...
     * @since 1.5
     */
    public usePattern(newPattern: Pattern): Matcher {
        if (newPattern == null)
            throw new IllegalArgumentException("Pattern cannot be null");
        this.parentPattern = newPattern;

        // Reallocate state storage
        let parentGroupCount = Math.max(newPattern.capturingGroupCount, 10);
        this.groups = new Array<number>(parentGroupCount * 2);
        this.locals = new Array<number>(newPattern.localCount);
        for (let i: number = 0; i < this.groups.length; i++)
            this.groups[i] = -1;
        for (let i: number = 0; i < this.locals.length; i++)
            this.locals[i] = -1;
        return this;
    }
     ...
     * @return  This matcher
     */
    public reset$java_lang_String(input: string): Matcher {
        this.text = input;
        return this.reset$();
    }

    public reset(input?: string): Matcher {
        if (input === undefined) {
            return this.reset$();
        }
     ...
     */
    public start$int(group: number): number {
        if (this.first < 0)
            throw new Error("No match available");
     ...
     * @since 1.8
     */
    public start$java_lang_String(name: string): number {
        return this.groups[this.getMatchedGroupIndex(name) * 2];
    }

    public start(arg?: string | number): number {
        if (arguments.length === 0) {
            return this.start$();
        }
        if (typeof arg === 'string') {
            return this.start$java_lang_String(<string>arg);
        }
        return this.start$int(<number>arg);
    }
     ...
     *          with the given index
     */
    public end$int(group: number): number {
        if (this.first < 0)
            throw new Error("No match available");
        if (group < 0 || group > this.groupCount())
            throw new IndexOutOfBoundsException("No group " + group);
     ...
     * @since 1.8
     */
    public end$java_lang_String(name: string): number {
        return this.groups[this.getMatchedGroupIndex(name) * 2 + 1];
    }

    public end(arg?: string | number): number {
        if (arguments.length === 0) {
            return this.end$();
        }
        if (typeof arg === 'string') {
            return this.end$java_lang_String(<string>arg);
        }
        return this.end$int(<number>arg);
    }
     ...
     *          with the given index
     */
    public group$int(group: number): string {
        if (this.first < 0)
     ...
     * @since 1.7
     */
    public group$java_lang_String(name: string): string {
        let group = this.getMatchedGroupIndex(name);
        if ((this.groups[group * 2] == -1) || (this.groups[group * 2 + 1] == -1))
            return null;
        return this.getSubSequence(this.groups[group * 2], this.groups[group * 2 + 1]).toString();
    }
    ...
    public matches(): boolean {
        return this.parentPattern.wholeRegex.test(this.text);
        // return this.match(this.from, Matcher.ENDANCHOR);
    }
     ...
     */
    public find$(): boolean {
        let nextSearchIndex: number = this.last;
     ...
     *          pattern
     */
    public find$int(start: number): boolean {
        let limit = this.getTextLength();
    ...
    public lookingAt(): boolean {
        return this.match(this.from, Matcher.NOANCHOR);
    }
     ...
     */
    public appendReplacement(sb: StringBuilder, replacement: string): Matcher {
        ...
        while (cursor < replacement.length) {
            let nextChar: string = replacement.charAt(cursor);
            if (nextChar == '\\') {
                cursor++;
                ...
                if (nextChar == '{') {
                    cursor++;
                    let gsb: StringBuilder = new StringBuilder();
                    while (cursor < replacement.length) {
                        nextChar = replacement.charAt(cursor);
                        if (ASCII.isLower(nextChar.charCodeAt(0)) ||
                            ASCII.isUpper(nextChar.charCodeAt(0)) ||
                            ASCII.isDigit(nextChar.charCodeAt(0))) {
                            gsb.append(nextChar);
                            cursor++;
                        } else {
                            break;
                    ...
                    let gname: string = gsb.toString();
                    if (ASCII.isDigit(gname.charCodeAt(0)))
                        throw new IllegalArgumentException(
                    ...
                    refNum = nextChar.charCodeAt(0) - '0'.charCodeAt(0);
                    ...
                    while (!done) {
                        ...
                        let nextDigit: number = replacement.charCodeAt(cursor) - '0'.charCodeAt(0);
                        if ((nextDigit < 0) || (nextDigit > 9)) { // not a number
                            break;
                        }
                        let newRefNum = (refNum * 10) + nextDigit;
                ...
                // Append group
                if (this.start$int(refNum) != -1 && this.end$int(refNum) != -1)
                    result.append(this.text, this.start$int(refNum), this.end$int(refNum));
            } else {
                result.append(nextChar);
                cursor++;
            }
     ...
     */
    public appendTail(sb: StringBuilder): StringBuilder {
     ...
     * @since 1.5
     */
    public region(start: number, end: number): Matcher {
     ...
     * @since 1.5
     */
    public useTransparentBounds(b: boolean): Matcher {
     ...
     * @since 1.5
     */
    public useAnchoringBounds(b: boolean): Matcher {
...
// }
    ...
    search(from: number): boolean {
        ...
        let result: boolean = this.parentPattern.root.match(this, from, this.text);
        if (!result)
            this.first = -1;
     ...
     */
    match(from: number, anchor: number): boolean {
        this.hitEnd = false;
        this.requireEnd = false;
        from = from < 0 ? 0 : from;
        this.first = from;
        ...
        let result: boolean = this.parentPattern.matchRoot.match(this, from, this.text);
        if (!result)
            this.first = -1;
        this.oldLast = this.last;
        return result;
     ...
     */
    getSubSequence(beginIndex: number, endIndex: number): string {
     ...
     */
    getMatchedGroupIndex(name: string): number {
        if (this.first < 0)
            throw new Error("No match found");
